...
for company in sample_companies:
    ...
    if result:
        ...
        cursor.execute("SELECT pl.net_profit, pl.sales, bs.equity_capital, bs.reserves, bs.borrowings FROM profitandloss pl JOIN balancesheet bs ON pl.company_id = bs.company_id AND pl.year = bs.year WHERE pl.company_id = ? AND pl.year = 2023", (company_id,))
        result = cursor.fetchall()

        if result:
            calculated_roe_values = []
            for row in result:
                net_profit, sales, equity_capital, reserves, borrowings = row
                if sales and sales != 0:
                    calculated_roe = ((net_profit or 0) / sales) * 100
                    calculated_roe_values.append(calculated_roe)

            if calculated_roe_values:
                avg_calculated_roe = sum(calculated_roe_values) / len(calculated_roe_values)
                difference = abs(stored_roe - avg_calculated_roe)
                percentage_diff = (difference / avg_calculated_roe) * 100 if avg_calculated_roe != 0 else 0
                status = "PASS" if percentage_diff <= 5 else "FAIL"

                roe_verification_results.append({
                    'company': company_name,
                    'stored_roe': stored_roe,
                    'calculated_roe': avg_calculated_roe,
                    'difference': difference,
                    'percentage_difference': percentage_diff,
                    'status': status
                })

                print(f"  Calculated ROE: {avg_calculated_roe:.2f}%")
