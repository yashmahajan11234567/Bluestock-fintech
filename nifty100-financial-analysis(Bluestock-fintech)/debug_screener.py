...
print('=== Manual walkthrough of load_screener_data with debugging ===')
...
print(f'   Company IDs: {companies["company_id"].head().tolist()}')
...
# Let's also check what's in the dataframe for ABB
print(f'\\nChecking ABB in final dataframe:')
abb_rows = df[df['company_id'] == 'ABB']
if len(abb_rows) > 0:
    abb_row = abb_rows.iloc[0]
    print(f'   ABB found!')
    print(f'   ROE: {abb_row.get("return_on_equity_pct", "MISSING")}')
    print(f'   Debt/EQ: {abb_row.get("debt_to_equity", "MISSING")}')
    print(f'   FCF: {abb_row.get("free_cash_flow_cr", "MISSING")}')
    print(f'   Sales CAGR: {abb_row.get("compounded_sales_growth", "MISSING")}')
    print(f'   Profit CAGR: {abb_row.get("compounded_profit_growth", "MISSING")}')
else:
    print(f'   ABB NOT FOUND!')